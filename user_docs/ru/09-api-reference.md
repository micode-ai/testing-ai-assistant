...
| Метод | Путь | Описание |
|-------|------|----------|
| `POST` | `/api/v1/users/batch` | Получить пользователей по массиву ID |

#### POST /api/v1/users/batch

Внутренний эндпоинт (`@Public()`, без JWT). Используется дашбордом для получения данных пользователей (например, имён участников на странице members).
